...
fn mcp_end_to_end() {
    ...
    // initialize
    write_framed(&mut stdin, &serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}));
    let resp = read_framed(&mut stdout);
    assert_eq!(resp.get("id").and_then(|x| x.as_i64()), Some(1));
    assert!(resp.get("result").is_some());

    // tools/list
    write_framed(&mut stdin, &serde_json::json!({"jsonrpc":"2.0","id":2,"method":"tools/list"}));
    let resp = read_framed(&mut stdout);
    ...
    // create_agent
    write_framed(&mut stdin, &serde_json::json!({
        "jsonrpc":"2.0","id":3,"method":"tools/call",
        "params": {"name":"create_agent","arguments":{"name":"t","args":[]}}
    }));
    let resp = read_framed(&mut stdout);
    let agent_id = resp.get("result").and_then(|r| r.get("content")).and_then(|c| c.get(0)).and_then(|j| j.get("json")).and_then(|j| j.get("agent_id")).and_then(|x| x.as_str()).unwrap().to_string();
    ...
    let resp = read_framed(&mut stdout);
    let content = resp.get("result").and_then(|r| r.get("content")).and_then(|c| c.get(0)).and_then(|j| j.get("json")).unwrap();
    assert_eq!(content.get("backend").and_then(|x| x.as_str()).unwrap(), "textrank");
    let summary = content.get("summary").and_then(|x| x.as_str()).unwrap().to_string();
